...
# A committed rendering of each phrase the suite speaks, so that the tier-2
# audio tests do not depend on an apt package. `integration/` installs from an
# offline mirror precisely so a build needs no network; gating its most valuable
# test on `sudo apt-get install espeak-ng` is the same class of external
# dependency by another name.
#
# The bytes are espeak-ng's own output, regenerated by `make speech-fixtures`,
# so a machine with espeak-ng installed and one without are speaking the same
# words. Stored unpadded: the lead-in and trail silence are VAD parameters, not
# properties of the recording, and belong to the caller.
FIXTURE_DIR = Path(__file__).resolve().parents[3] / "fixtures" / "speech"


def fixture_name(text: str) -> str:
    """The file name a phrase is committed under."""
    return re.sub(r"[^a-z0-9]+", "-", text.lower()).strip("-") or "utterance"


def fixture_path(text: str) -> Path:
    return FIXTURE_DIR / f"{fixture_name(text)}.wav"


def can_speak(text: str) -> bool:
    """Whether :func:`speech_for` can render *text* on this machine.

    The skip condition for a test that needs real speech: espeak-ng renders any
    phrase, a fixture covers only the ones that were committed.
    """
    return speech_available() or fixture_path(text).exists()


def speech_for(text: str, words_per_minute: int = ESPEAK_WPM,
               lead_ms: int = LEAD_SILENCE_MS,
               trail_ms: int = TRAIL_SILENCE_MS) -> np.ndarray:
    """Real speech saying *text*, from espeak-ng if it is installed, else from disk.

    Prefer this to :func:`spoken` in tests. Use :func:`spoken` only where the
    phrase is chosen at runtime and a fixture could not exist — the demo
    launcher's ``--say``.
    """
    if speech_available():
        return spoken(text, words_per_minute, lead_ms, trail_ms)

    path = fixture_path(text)
    if not path.exists():
        raise RuntimeError(
            f"{ESPEAK} is not installed and no committed fixture says {text!r} "
            f"(looked for {path}). Install it with `sudo apt-get install -y "
            f"{ESPEAK}`, or add the phrase to cltl_integration.fixtures and run "
            f"`make speech-fixtures`.")

    return np.concatenate((silence(lead_ms), _read_mono_16k(path),
                           silence(trail_ms)))


def write_fixture(text: str) -> Path:
    """Render *text* with espeak-ng and commit it, unpadded. Used by `make speech-fixtures`."""
    path = fixture_path(text)
    path.parent.mkdir(parents=True, exist_ok=True)

    import soundfile

    soundfile.write(str(path), spoken(text, lead_ms=0, trail_ms=0), RATE,
                    subtype="PCM_16")
...
class StubAudioServer:
    ...
    def await_connection(self, count: int = 1, timeout: float = 15.0) -> int:
        """Block until the backend's microphone thread has dialled in *count* times.

        Recording starts on a ScenarioStarted event, so anything that depends on
        the microphone actually being open — the audio resource being held, a
        recording existing to fetch — has to wait for the connection rather than
        ...
        while self.served < count:
            if time.monotonic() >= deadline:
                raise AssertionError(
                    f"the microphone connected {self.served} time(s) in {timeout}s, "
                    f"expected {count}")
            time.sleep(0.05)
